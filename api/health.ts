...
async function checkAIProviders(): Promise<'healthy' | 'unhealthy' | 'degraded'> {
  const providers = [
    ...
    { name: 'Google', key: process.env.GOOGLE_API_KEY, url: 'https://generativelanguage.googleapis.com/v1beta/models' },
  ];
  ...
  for (const provider of providers) {
    ...
    try {
      const response = await fetch(provider.url, {
        method: 'GET',
        headers: {
          'Authorization': provider.name === 'Google' ? 
            `Bearer ${provider.key}` : 
            `Bearer ${provider.key}`,
      ...
      if (response.status < 500) { // Consider 4xx as healthy (auth issues are expected in health checks)
        healthyCount++;
      }
    } catch (error) {
...
async function checkExternalAPIs(): Promise<'healthy' | 'unhealthy' | 'degraded'> {
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const [aiProvidersHealth, storageHealth, externalAPIsHealth] = await Promise.all([
      checkAIProviders(),
      checkStorage(),
      checkExternalAPIs(),
    ]);
    ...
    const services = { ai_providers: aiProvidersHealth, storage: storageHealth, external_apis: externalAPIsHealth };
