...
export const useActiveChat = () => {
  return useChatStore(
    useCallback((state) => ({
      ...
      messages: state.activeChat ? state.messages[state.activeChat] || [] : [],
...
export const useSelectedModel = () => {
  return useModelStore(
    useCallback((state) => {
      ...
      const model = state.getModel(selectedModel.provider, selectedModel.modelId);
      const config = state.getModelConfig(selectedModel.provider, selectedModel.modelId);
...
export const useModelConfig = (provider: AIProvider, modelId: string) => {
  return useModelStore(
    useCallback((state) => ({
      ...
      updateConfig: (updates: Parameters<typeof state.updateModelConfig>[2]) =>
...
export const useMCPServers = () => {
  return useMCPStore(
    useCallback((state) => {
      ...
      return {
        servers: state.selectedTab === 'builtin' ? filtered.builtin : filtered.custom,
...
export const useModelWithConfig = (provider: AIProvider, modelId: string) => {
  ...
  const { config, updateConfig, resetConfig } = useModelConfig(provider, modelId);
