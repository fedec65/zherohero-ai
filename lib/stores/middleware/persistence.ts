...
// Migration utilities
export function createMigration<T>(migrations: Record<number, (state: unknown) => T>) {
  return (persistedState: unknown, version: number): T => {
    let state = persistedState;
    
    for (let v = version; v < Math.max(...Object.keys(migrations).map(Number)); v++) {
...
export function createPartializer<T>(excludeKeys: (keyof T)[]): (state: T) => Partial<T> {
  return (state: T) => {
...
export function createDebouncedStorage(storage: StateStorage, delay: number = 500): StateStorage {
