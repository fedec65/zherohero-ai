...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (body.stream) {
      response = await handleStreamingRequest(body.provider, params, {
        requestId,
        abortController,
        timeoutId,
      })
    } else {
      response = await handleNonStreamingRequest(body.provider, params, {
        requestId,
        startTime,
      })
    }
    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        try {
          ...
          for await (const chunk of aiAPI.streamChatCompletion(
            provider,
            params
          )) {
...
function extractApiKeysFromHeaders(
  headers: Headers
