...
export async function GET() {
  try {
    ...
    const providerDetails = providers.reduce(
      (acc, provider) => {
    ...
    const initializedCount = Object.values(statuses).filter(
      (s) => s.initialized,
    ).length;
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Test connection
    const testResult = await aiAPI.testProviderConnection(
      provider,
      testMessage,
    );
