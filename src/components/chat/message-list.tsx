...
export function MessageList({ chatId, className }: MessageListProps) {
  ...
  const handleScroll = useCallback((props: any) => {
    ...
    const maxScroll = Math.max(0, (chatMessages.length * 100) - containerHeight); // Rough estimation
  ...
  const MessageRow = React.memo(({ index, style, data }: ListChildComponentProps & { data: any }) => {
    const { messages: msgs, streamingMessage: streaming, isStreamingInThisChat: isStreamingChat } = data;
    const message = msgs[index];
    
    if (!message) return null;

    const isStreaming = isStreamingChat && streaming?.messageId === message.id;
    ...
    return (
      <div style={style} className="px-4">
        <MessageItem
          ...
          streamingContent={isStreaming ? streaming?.content || '' : undefined}
        />
      </div>
    );
  });
