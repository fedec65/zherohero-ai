...
export function ThemeToggle({ variant = 'icon', className }: ThemeToggleProps) {
  ...
  const getIcon = () => {
    ...
    switch (themeToShow) {
      case 'light':
        return <Sun className="h-4 w-4 transition-transform duration-200 hover:rotate-12" />;
      case 'dark':
        return <Moon className="h-4 w-4 transition-transform duration-200 hover:rotate-12" />;
      default:
        return <Sun className="h-4 w-4 transition-transform duration-200 hover:rotate-12" />;
    }
  };
