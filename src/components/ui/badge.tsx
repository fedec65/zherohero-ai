...
function Badge({
  ...
  return (
    <div
      ...
      {...props}
    >
      {leftIcon && <span className="-ml-0.5 mr-1">{leftIcon}</span>}
      {children}
      {rightIcon && <span className="-mr-0.5 ml-1">{rightIcon}</span>}
    </div>
  )
}
...
function StatusBadge({ status, children, ...props }: StatusBadgeProps) {
  ...
  return (
    <Badge
      ...
      leftIcon={<div className="h-2 w-2 rounded-full bg-current" />}
      {...props}
    >
...
function CountBadge({
  ...
  return (
    <Badge
      ...
      size="sm"
      className={clsx(
        'flex h-5 min-w-[1.25rem] items-center justify-center px-1.5',
        className
      )}
      {...props}
    >
