...
export class OpenAIClient implements BaseAPIClient {
  ...
  async createChatCompletion(
    ...
    try {
      ...
      if (duration > 5000) {
        console.warn(
          `Slow OpenAI request: ${params.model} - ${duration.toFixed(2)}ms`,
        );
      }
      ...
      try {
        for await (const chunk of stream as any) {
          ...
          if (transformedChunk) {
            ...
            if (
              firstChunkTime === null &&
              transformedChunk.choices?.[0]?.delta?.content
            ) {
              ...
              if (firstTokenLatency > 3000) {
                console.warn(
                  `Slow first token: ${params.model} - ${firstTokenLatency.toFixed(2)}ms`,
                );
              }
            }
        ...
        if (totalDuration > 30000) {
          // > 30 seconds
          console.warn(
            `Long streaming request: ${params.model} - ${totalDuration.toFixed(2)}ms, ${chunkCount} chunks`,
          );
        }
      }
    } catch (error) {
