...
export interface OpenRouterErrorDetails {
  ...
  userMessage: string;
  retryable: boolean;
  action?: 'check_key' | 'check_balance' | 'try_different_model' | 'wait' | 'contact_support';
}
...
const OPENROUTER_ERROR_CODES: Record<string, OpenRouterErrorDetails> = {
  ...
  'invalid_api_key': {
    ...
    userMessage: 'Your OpenRouter API key is invalid. Please check your API key in Settings.',
    retryable: false,
    action: 'check_key'
  },
  'insufficient_balance': {
    ...
    userMessage: 'Insufficient OpenRouter balance. Please add funds to your OpenRouter account.',
    retryable: false,
  ...
  'rate_limited': {
    ...
    userMessage: 'Too many requests to OpenRouter. Please wait a moment and try again.',
    retryable: true,
    action: 'wait'
  },
  'daily_limit_exceeded': {
    ...
    userMessage: 'Daily OpenRouter limit exceeded. Try again tomorrow or upgrade your plan.',
    retryable: false,
    action: 'check_balance'
  },
  ...
  'model_not_found': {
    ...
    userMessage: 'This model is not available on OpenRouter. Please try a different model.',
    retryable: false,
  ...
  'model_offline': {
    ...
    userMessage: 'This model is temporarily unavailable. Please try another model or wait a few minutes.',
    retryable: true,
    action: 'try_different_model'
  },
  'model_overloaded': {
    ...
    userMessage: 'This model is experiencing high demand. Please try again in a few minutes or use a different model.',
    retryable: true,
    action: 'wait'
  },
  ...
  'invalid_request_format': {
    ...
    userMessage: 'There was an issue with your request format. Please try again.',
    retryable: false,
  ...
  'context_length_exceeded': {
    ...
    userMessage: 'Your message is too long for this model. Please shorten your message or use a model with a larger context window.',
    retryable: false,
    action: 'try_different_model'
  },
  'content_filtered': {
    ...
    userMessage: 'Your request was filtered due to content policy. Please modify your message and try again.',
    retryable: false
  },
  ...
  'internal_server_error': {
    ...
    userMessage: 'OpenRouter is experiencing technical difficulties. Please try again in a few minutes.',
    retryable: true,
    action: 'wait'
  },
  'service_unavailable': {
    ...
    userMessage: 'OpenRouter is temporarily unavailable. Please try again in a few minutes.',
    retryable: true,
    action: 'wait'
  },
  'timeout': {
...
export function parseOpenRouterError(error: any): OpenRouterErrorDetails {
  ...
  if (error.message?.includes('fetch')) {
    return {
      ...
      userMessage: 'Unable to connect to OpenRouter. Please check your internet connection and try again.',
      retryable: true
    };
  }
  ...
  for (const [code, details] of Object.entries(OPENROUTER_ERROR_CODES)) {
    if (errorMessage.includes(code.replace('_', ' ')) || 
 ...
 */
export function shouldRetry(error: OpenRouterErrorDetails, attemptCount: number): boolean {
  if (!error.retryable || attemptCount >= 3) {
    return false;
  }

  // Don't retry authentication or balance errors
  if (['invalid_api_key', 'insufficient_balance', 'daily_limit_exceeded'].includes(error.code)) {
    return false;
  }

  // Retry server errors and rate limits with exponential backoff
  if (['internal_server_error', 'service_unavailable', 'rate_limited', 'timeout'].includes(error.code)) {
    return true;
  }
...
export function getRetryDelay(attemptCount: number, errorCode: string): number {
  ...
  const delay = Math.min(baseDelay * Math.pow(2, attemptCount) * multiplier, maxDelay);
