...
interface MCPActions {
  // Server management
  addCustomServer: (server: Omit<MCPServer, 'id' | 'createdAt' | 'updatedAt' | 'status'>) => Promise<string>;
  ...
  performHealthCheck: (serverId: string) => Promise<{ healthy: boolean; latency: number; capabilities: MCPCapability[] }>;
  ...
  getServerStats: (serverId: string) => { requests: number; successes: number; failures: number; avgResponseTime: number };
  resetStats: () => void;
  ...
  // Utilities
  validateServerConfig: (server: Partial<MCPServer>) => { isValid: boolean; errors: string[] };
      ...
      immer((set, get) => ({
        ...
        updateServer: (serverId: string, updates: Patch<MCPServer>) => {
          set((state) => {
            ...
            const builtInIndex = state.builtInServers.findIndex(s => s.id === serverId);
            if (builtInIndex >= 0) {
              Object.assign(state.builtInServers[builtInIndex], updates, { updatedAt: new Date() });
              return;
            }

            // Check custom servers
            const customIndex = state.customServers.findIndex(s => s.id === serverId);
            if (customIndex >= 0) {
              Object.assign(state.customServers[customIndex], updates, { updatedAt: new Date() });
            }
          });
        },
        ...
        deleteServer: (serverId: string) => {
          set((state) => {
            ...
            state.customServers = state.customServers.filter(s => s.id !== serverId);
        ...
        connectServer: async (serverId: string) => {
          ...
          try {
            ...
            if (currentServer) {
              get().updateServer(serverId, { ...currentServer, status: 'error' });
            }
            throw error;
          }
        },
        ...
        disconnectServer: async (serverId: string) => {
          try {
            ...
            const currentServer = state.builtInServers.find(s => s.id === serverId) || 
                                 state.customServers.find(s => s.id === serverId);
            if (currentServer) {
              get().updateServer(serverId, { ...currentServer, status: 'disconnected' });
            }
          } catch (error) {
        ...
        updateServerConfig: (serverId: string, config: Record<string, unknown>) => {
          const state = get();
          const currentServer = state.builtInServers.find(s => s.id === serverId) || 
                               state.customServers.find(s => s.id === serverId);
          if (currentServer) {
        ...
        // Global settings
        updateGlobalSettings: (settings: Partial<MCPState['globalSettings']>) => {
          set((state) => {
        ...
        broadcastRequest: async (request: MCPRequest, capability: MCPCapability) => {
          const serversWithCapability = get().getServersByCapability(capability)
            .filter(server => server.enabled && get().isServerHealthy(server.id));
        ...
        importServers: async (file: File) => {
          ...
          const serversToImport: MCPServer[] = importData.servers.map((serverData: any) => ({
            ...serverData,
      ...
      })),
      {
        ...
        onRehydrateStorage: () => (state) => {
          if (state) {
            ...
            [...state.builtInServers, ...state.customServers].forEach(server => {
