...
describe('TavilySearchClient', () => {
  ...
  describe('API Key Validation', () => {
    it('should accept valid Tavily API key format', () => {
      expect(() => {
        new TavilySearchClient({ apiKey: 'tvly-abcd1234567890abcd1234567890abcd' });
      }).not.toThrow();
    });
  ...
  describe('Search Functionality', () => {
    ...
    it('should handle API errors gracefully', async () => {
      ...
      await expect(client.search({ query: 'test' }))
        .rejects
        .toThrow(TavilyError);
    });
  ...
  describe('Content Extraction', () => {
    ...
    it('should validate URL array length', async () => {
      ...
      await expect(client.extract({ urls: tooManyUrls }))
        .rejects
        .toThrow('Maximum 5 URLs allowed');
    });

    it('should require non-empty URL array', async () => {
      await expect(client.extract({ urls: [] }))
        .rejects
        .toThrow('URLs array cannot be empty');
    });
  });
...
describe('TavilyMCPServer', () => {
  ...
  beforeEach(() => {
    ...
    (TavilySearchClient.prototype.testConnection as jest.Mock) = jest.fn()
  ...
  describe('Server Initialization', () => {
    ...
    it('should fail initialization without API key', async () => {
      ...
      await expect(serverWithoutKey.initialize())
        .rejects
  ...
  describe('Tool Execution', () => {
    beforeEach(async () => {
      ...
      (TavilySearchClient.prototype.search as jest.Mock) = jest.fn()
        .mockResolvedValue({
          query: 'test query',
  ...
  describe('Resources', () => {
    ...
    it('should provide search history resource', async () => {
      ...
      const historyResource = resources.find(r => r.uri === 'tavily://search-history');
    ...
    it('should handle unknown resources', async () => {
      await expect(server.getResource('tavily://unknown'))
        .rejects
...
describe('Store Integration', () => {
  describe('MCP Store', () => {
    it('should have Tavily server in built-in servers', () => {
      ...
      const tavilyServer = mcpStore.builtInServers.find(s => s.id === 'tavily-search');
  ...
  describe('Settings Store', () => {
    it('should validate Tavily API key format', async () => {
      ...
      const validKey = await settingsStore.validateApiKey('tavily', 'tvly-abcd1234567890abcd1234567890abcd');
      expect(validKey).toBe(true);
      
      const invalidKey = await settingsStore.validateApiKey('tavily', 'invalid-key');
