...
test.describe('MCP Servers Page', () => {
  ...
  test('should display MCP server tabs', async ({ page }) => {
    ...
    await expect(
      page.locator('button', { hasText: 'Built-in Servers' })
    ).toBeVisible()
    await expect(
      page.locator('button', { hasText: 'Custom Servers' })
    ).toBeVisible()
    await expect(
      page.locator('button', { hasText: 'Add Custom Server' })
    ).toBeVisible()
  })
  ...
  test('should show server status indicators', async ({ page }) => {
    ...
    await expect(
      serverCard.locator('[data-testid="server-status"]')
    ).toBeVisible()
  })
