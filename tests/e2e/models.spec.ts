...
test.describe("Models Page", () => {
  ...
  test("should display model tabs", async ({ page }) => {
    ...
    await expect(page.locator("button", { hasText: "Built-in Models" })).toBeVisible();
    await expect(page.locator("button", { hasText: "Custom Models" })).toBeVisible();
    await expect(page.locator("button", { hasText: "Add Custom Model" })).toBeVisible();
    await expect(page.locator("button", { hasText: "OpenRouter" })).toBeVisible();
  });
  ...
  test("should show model details", async ({ page }) => {
    ...
    await expect(firstModelCard).toContainText(/GPT|Claude|Gemini|Grok|DeepSeek/);
    
    // Check for configure button
    const configureButton = firstModelCard.locator("button", { hasText: "Configure" });
